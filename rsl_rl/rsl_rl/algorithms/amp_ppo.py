...
"""
对抗性运动先验 (Adversarial Motion Priors, AMP) 近端策略优化算法模块

本模块实现了结合AMP的PPO算法，用于学习自然和人类化的机器人运动。
AMP通过对抗性训练来学习运动先验，PPO则用于策略优化。
...
- AMP: Adversarial Motion Priors for Learning Natural Robot Behaviors
"""
...
class AMPPPO:
    ...
    该类实现了结合AMP的PPO算法，通过对抗性训练学习自然运动，
    同时使用PPO进行策略优化。
    
    属性:
        policy: Actor-Critic策略网络
        discriminator: AMP判别器网络
        rnd: 随机网络蒸馏模块（可选）
        symmetry: 对称性增强配置（可选）
    """
    ...
    def __init__(
        ...
        Args:
            ...
            discriminator: AMP判别器网络
            amp_data: AMP专家数据加载器
            amp_normalizer: AMP数据归一化器
            amp_replay_buffer_size: AMP回放缓冲区大小，默认为100000
            min_std: 最小动作标准差，默认为None
        ...
        # 判别器组件
        self.amploss_coef = 1.0  # AMP损失系数
        self.min_std = min_std
        self.discriminator = discriminator
        self.discriminator.to(self.device)
        self.amp_transition = RolloutStorage.Transition()  # AMP转换存储
        self.amp_storage = ReplayBuffer(discriminator.input_dim // 2, amp_replay_buffer_size, device)  # AMP回放缓冲区
        self.amp_data = amp_data  # AMP专家数据
        self.amp_normalizer = amp_normalizer  # AMP归一化器
        ...
        # 创建优化器
        params = [
            {"params": self.policy.parameters(), "name": "policy"},  # 策略网络参数
            {"params": self.discriminator.trunk.parameters(), "weight_decay": 10e-4, "name": "amp_trunk"},  # 判别器主干参数
            {"params": self.discriminator.amp_linear.parameters(), "weight_decay": 10e-2, "name": "amp_head"},  # 判别器头部参数
        ]
